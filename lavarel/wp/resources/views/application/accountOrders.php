    ...
    <table border="0">
        ...
        ?>
            ...
            ?></span> <?php echo $object->body_name; ?> <?php echo $order->body_time;?> 秒</strong>
                    <p>已于 <?php echo date('Y-m-d H:i:s', strtotime($order->created_at)); ?> 以 <?php echo $order->body_price_buying; ?> 买入 <?php echo $order->body_stake;?> 元 <?php echo $order->body_direction ? '看涨' : '看跌'; ?></p>
                    <?php if ($is_striked) { ?>
                    <p>已于 <?php echo date('Y-m-d H:i:s', strtotime($order->created_at) + intval($order->body_time)); ?> 以 <?php echo $order->body_price_striked; ?> 结算</p>
                    <?php } else { ?>
                    <p>将于 <?php echo date('Y-m-d H:i:s', strtotime($order->created_at) + intval($order->body_time)); ?> 结算</p>
                    <?php } ?>
                </td>
            </tr>
        <?php } ?>

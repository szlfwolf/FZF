...
Tooltip.prototype = {
	 ...
	 * @private
	 */
	move: function (x, y, anchorX, anchorY, isOwnAnimate) {
		var tooltip = this,
			now = tooltip.now,
			animate = tooltip.options.animation !== false && !tooltip.isHidden && isOwnAnimate &&
		...
		if (animate) {
			...
			this.tooltipTimeout = setTimeout(function () {
				...
				if (tooltip) {
					tooltip.move(x, y, anchorX, anchorY,isOwnAnimate);
				}
			}, 32);
	...
	refresh: function (point, mouseEvent) {
		...
		if (text === false) {
            ...
            //增加对tooltip位置的处理
            var tooltip_width = 130;
            var oldTargetX = 5;
            var targetY = 25;

            if(x <= oldTargetX + tooltip_width + 10)
            {
                var targetX = window_width - tooltip_width;
                tooltip.updatePosition({ x:oldTargetX,y:targetY,plotX: targetX, plotY: targetY,isOwnAnimate:true });
            }
            else if(x >= window_width - tooltip_width - oldTargetX - 10)
            {
                tooltip.updatePosition({ x:window_width - tooltip_width,y:targetY,plotX: oldTargetX, plotY: targetY, isOwnAnimate:true });
            }
	...
	updatePosition: function (point) {
		...
		// do the move
        if(point.isOwnAnimate != undefined && point.isOwnAnimate != 'undefined')
        {
            this.move(
                point.plotX + chart.plotLeft,
                point.plotY + chart.plotTop,
                mathRound(point.x),
                mathRound(point.y), // can be undefined (#3977)
                point.isOwnAnimate
            );
        }
        else

...
class AgentController extends Controller {
    ...
    public function withholdForUser(Request $request, $id) {
        ...
        return view('agent.withholdForUser', [
            'active' => 'users',
            'id_user' => $id,
            'alert' => $alert
        ]);
    ...
    public function withdrawForUser(Request $request, $id) {
        ...
        return view('agent.withdrawForUser', [
    ...
    public function withdrawForUserCanceled(Request $request, $id) {
        ...
        return redirect('/agent/withdrawRequests');
    ...
    public function withdrawRequests(Request $request) {
        ...
        return view('agent.withdrawRequests', [
            'active' => 'withdrawRequests',
            'datas' => $datas,
            'id_user' => $request->input('id_user')
        ]);
    ...
    public function administrators(Request $request) {
        $this->requiredSession($request);
        $datas = Agent::orderBy('created_at', 'desc')->paginate(20);
        return view('agent.administrators', [
    ...
    public function signIn(Request $request) {
        ...
        if ($request->isMethod('post')) {
            $agent = Agent::where('body_email', $request->input('email'))->where('body_password', md5($request->input('password')))->first();
            if($agent){
                $request->session()->put('agent', $agent->id);
                return redirect('/agent');
            }
        }
        return view('agent.signIn');
    }
    ...
    public function ordersControll(Request $request, $id) {

        $this->requiredSession($request);

        $order = Order::find($id);

        if ($order->striked_at == '0000-00-00 00:00:00' && $order->body_is_controlled == 0){

            $order_striked_time = strtotime($order->created_at) + $order->body_time;
            $price = $this->computePriceItem($order_striked_time, $order, $order->object, mt_rand(1, 3));

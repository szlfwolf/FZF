...
class ApiController extends Controller {
    ...
    public function fetch() {
        ...
        $btcObject = Object::find(13);
        $btcContents = file_get_contents('http://api.huobi.com/staticmarket/ticker_btc_json.js');
        if(!Cache::has(md5($btcContents))){
            Cache::put(md5($btcContents),'FETCHED',4320);
            $btc = json_decode($btcContents);
            $btc = $btc->ticker;
            print_r($btc);
            $this->insertPrice($btcObject,$btc->buy, date('Y-m-d H:i:s', time()),$btc->buy,$btc->buy,$btc->sell);
    ...
    public function compute() {
        ...
        foreach($orders as $order){
            ...
            $order_id = $order->id;
            if($order_striked_time <= time() && $order->striked_at == '0000-00-00 00:00:00'){
                ...
                if($order->body_is_win == 1){
                    ...
                    $myorder->body_balance = $user->body_balance;
                    $myorder->save();
                    $record = new Record;
                    ...
                    $record->id_member = $id_member;
                    $record->body_stake = $order->body_stake + $order->body_bonus;
                    $record->save();

                    if(!empty($id_member)){
                        //说明所属会员发展进来的客户，需要对会员进行手续费扣除，同时资金进行对冲
                        $agent = User::find($id_member);
                        //扣除手续费，资金对冲，生成日志
                        $rate_momney = $order->body_bonus * $agent->rate /100;
                        $fee_momney = $order->body_stake * $agent->fee /100;
                        $agent->body_balance = $agent->body_balance - $rate_momney - $fee_momney;

                        $agent->save();
                        

                        //手续费日志
                        $fee_record = new Record;
                        $fee_record->id_user = $id_member;
                        $fee_record->id_order = $order->id;
                        $fee_record->id_member = $id_member;
                        $fee_record->body_name = '用户交易扣除手续费';
                        $fee_record->body_direction = 0;
                        $fee_record->body_stake = $fee_momney;
                        $fee_record->save();


                        //对冲费用扣除日志
                        $rate_record = new Record;
                        $rate_record->id_user = $id_agent;
                        $rate_record->id_order = $order->id;
                        $rate_record->id_member = $id_member;
                        $rate_record->body_name = '用户盈利，对冲支出';
                        $rate_record->body_direction = 0;
                        $rate_record->body_stake = $rate_momney;
                        $rate_record->save();
                    }
                ...
                if($order->body_is_win == 0 && $order->body_is_draw != 1 ){
                    //用户数钱，扣除机构手续费，加上对冲金额
                     if(!empty($id_member)){
                        //说明是会员发展进来的客户，需要对会员进行手续费扣除，同时资金进行对冲
                        DB::beginTransaction();
                        $agent = User::find($id_member);
                        //扣除手续费，资金对冲，生成日志
                        $rate_momney = $order->body_stake * $agent->rate /100;
                        $fee_momney = $order->body_stake * $agent->fee /100;
                        $agent->body_balance = $agent->body_balance + $rate_momney - $fee_momney;
                        ...
                        $rate_record->id_order = $order->id;
                        $rate_record->id_member = $id_member;
                        $rate_record->body_name = '用户亏损，对冲收入';
                        $rate_record->body_direction = 1;
                        $rate_record->body_stake = $rate_momney;
                        $rate_record->save();
                    }
                ...
                $myuser = User::find($myorder->id_user);
                $myorder->body_balance = $myuser->body_balance;
                $myorder->save();
                DB::commit();
    ...
    public function captchaCreate(Request $request, Response $response) {
        ...
        $requestUri = str_replace('#3#', urlencode("【云交易】你的验证码是" . $code . "，请在10分钟内输入。"), $requestUri);
    ...
    public function orderCreate(Request $request, Response $response) {
        ...
        $carbon = Carbon::now();
        if($carbon->hour < 6 && $carbon->hour > 4) {
            return response()->json([
                'error' => '非交易时间无法进行交易'
            ]);
        }
        ...
        if($user->body_balance < 0) {
            ...
            $order->id_broker = $user->id_broker;
            $order->id_agent = $user->id_agent;
            $order->save();
            ...
            $record->body_stake = $order->body_stake;
            $record->id_member = $user->id_member;
            $record->save();
